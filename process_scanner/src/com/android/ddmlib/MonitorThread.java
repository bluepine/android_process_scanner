...
final class MonitorThread extends Thread {
	...
	/**
	 * "Selected" client setup to answer debugging connection to the
	...
	synchronized void setDebugSelectedPort(int port)
		...
		if (mDebugSelectedChan != null) {
			...
			// we set mNewDebugSelectedPort instead of mDebugSelectedPort so
	...
	/**
	 * Returns the client accepting debugger connection on the custom
	 * "Selected debug port".
	 */
	...
	public void run() {
		...
		while (!mQuit) {
			...
			try {
				/*
				 * sync with new registrations: we wait until addClient is done
				 * before going through and doing mSelector.select() again.
				 * 
				...
				// (re-)open the "debug selected" port, if it's not opened yet
				// or
				// if the port changed.
				try {
					if (AndroidDebugBridge.getClientSupport()) {
						if ((mDebugSelectedChan == null || mNewDebugSelectedPort != mDebugSelectedPort)
					...
					Log.e("ddms",
							"Failed to reopen debug port for Selected Client to: "
				...
				while (iter.hasNext()) {
					...
					try {
						...
						// we don't want to have our thread be killed because of
						// any uncaught
						// exception, so we intercept all here.
						Log.e("ddms",
								"Exception during activity from Selector.");
						Log.e("ddms", e);
					}
				}
			} catch (Exception e) {
				// we don't want to have our thread be killed because of any
				// uncaught
				// exception, so we intercept all here.
	...
	private void processClientActivity(SelectionKey key) {
		...
		try {
			if (key.isReadable() == false || key.isValid() == false) {
				Log.d("ddms", "Invalid key from " + client
						+ ". Dropping client.");
			...
			// key was canceled probably due to a disconnected client before we
			// could
			// read stuff coming from the client, so we drop it.
			dropClient(client, true /* notify */);
		} catch (IOException ex) {
			// something closed down, no need to print anything. The client is
			// simply dropped.
			dropClient(client, true /* notify */);
		} catch (Exception ex) {
			Log.e("ddms", ex);
	...
	private void callHandler(Client client, JdwpPacket packet,
		...
		if (handler == null) {
			Log.w("ddms",
					"Received unsupported chunk type "
	 ...
	 * <p/>
	 * This will lock the {@link Client} list of the {@link Device} running
	 * <var>client</var>.
	 * 
	 * @param client
	 * @param notify
		...
		try {
			...
			if (client.isDdmAware()) {
				...
				client.getDeviceImpl()
						.getMonitor()
						.addClientToDropAndReopen(client,
	...
	private boolean reopenDebugSelectedPort() throws IOException {
		...
		InetSocketAddress addr = new InetSocketAddress(
				...
				mNewDebugSelectedPort);
		mDebugSelectedChan.socket().setReuseAddress(true); // enable
															// SO_REUSEADDR
		...
		try {
			...
			mDebugSelectedChan
					.register(mSelector, SelectionKey.OP_ACCEPT, this);
	...
	private void displayDebugSelectedBindError(int port) {
		String message = String
				.format("Could not open Selected VM debug port (%1$d). Make sure you do not have another instance of DDMS or of the eclipse plugin running. If it's being used by something else, choose a new port number in the preferences.",
						port);
